...
import { MapPin, Calendar, MessageCircle, MessageSquare } from "lucide-react";
      ...
      >
        ...
        <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: '1rem' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.875rem', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <MapPin size={16} className="text-gray-500" /> {chantier.adresse}
            </p>
          </div>
          ...
          <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ color: '#64748b', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Calendar size={14} /> {formatDate(chantier.dateDebut)} - {formatDate(chantier.dateFin)}
              </span>
            ...
            {chantier._count && (
              <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                <span style={{ color: '#64748b', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <MessageCircle size={14} /> {chantier._count.messages} messages
                </span>
                <span style={{ color: '#64748b', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <MessageSquare size={14} /> {chantier._count.comments} commentaires
                </span>
              </div>
            )}
          </div>
