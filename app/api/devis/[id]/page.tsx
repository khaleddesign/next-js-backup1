...
import PaiementTracker from '@/components/factures/PaiementTracker';
import RelanceManager from '@/components/factures/RelanceManager';
   ...
   <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
     <div style={{ 
       ...
       <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
         ...
         {/* Bouton Aperçu pour tous les statuts sauf BROUILLON */}
         {devis.statut !== 'BROUILLON' && (
           <button
             onClick={() => setShowPreview(true)}
             style={{
     ...
     {/* Modules spécifiques aux factures */}
     {devis.type === 'FACTURE' && (
       <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
         <PaiementTracker 
           factureId={devis.id}
           montantTotal={Number(devis.totalTTC)}
           statut={devis.statut}
         />
         ...
         {devis.statut === 'ENVOYE' && (
           <RelanceManager
             factureId={devis.id}
             factureNumero={devis.numero}
             clientName={devis.client.name}
             montantDu={Number(devis.totalTTC)}
           />
         )}
       </div>
     )}
     ...
     <DevisPreviewModal
       ...
       devis={devis}
       onSend={devis.statut === 'BROUILLON' ? () => handleAction('send') : undefined}
       onDownload={() => alert('Téléchargement PDF simulé')}
     />
   </div>
 );
}