...
export default function UploadPage() {
  ...
  const handleUpload = async (files: FileList, metadata: any) => {
    ...
    try {
      ...
      if (response.ok) {
        // Assurons-nous que result contient toujours les propriétés attendues
        const safeResult = {
        ...
        setUploadResults(safeResult);
        success(
          'Upload terminé', 
          `${safeResult.uploaded}/${safeResult.total} fichier(s) uploadé(s) avec succès`
        );
      ...
      // Réinitialisons uploadResults pour éviter les états incohérents
      setUploadResults({ success: [], errors: [] });
    } finally {
      setUploading(false);
    }
                    ...
                    {(uploadResults?.success || []).map((doc: any, index: number) => (
                      <div key={index} className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                        <div className="flex items-center justify-between">
                          <span className="text-green-200">{doc?.nomOriginal || 'Fichier'}</span>
                          <span className="text-green-300 text-sm">{doc?.type || 'Document'}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    {(uploadResults?.errors || []).map((error: any, index: number) => (
                      <div key={index} className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                        <div className="flex items-center justify-between">
                          <span className="text-red-200">{error?.fileName || 'Fichier inconnu'}</span>
                          <span className="text-red-300 text-sm">{error?.error || 'Erreur inconnue'}</span>
                        </div>
                      </div>
                    ))}
                  </div>
